...
static void find_server(void) {
  ...
  if (!h) {
    fprintf(stderr,"Error: Can't resolve server address.\n");
    exit(1);
  }
...
static int contact_server(void) {
  ...
  if (!select(fd + 1, &fds, NULL, NULL, &tv)) {
    fprintf(stderr,"No reply within %i second%s.\n", ttime, ttime == 1 ? "" : "s");
    return 1;
  }

  if (recv_packet(packet, 256, 0, 0, 1, 0, 0) < 0) {
    if (errno == EINTR) {
      fprintf(stderr, "Server hung up.\n");
      return 1;
    } else {
  ...
  while((a = getopt(argc, argv, "hvTp:a:t:")) != -1) {
    switch(a) {
      ...
      case 't':
        ...
        if (ttime < 1) {
          fprintf(stderr, "Error: Timeout too low.\n");
          exit(1);
        }
        break;
