...
my_putc(char c) {
  ...
  if (screen_buffer_pos + 1 > screen_buffer_size) {
    ...
    if (!screen_buffer) {
      c_shutdown();
      fprintf(stderr, "Error: Can't reallocate screen buffer.\n");
      EXIT(1);
    }
  }
...
void c_init(int w, int h) {
  ...
  if (!(screen_buffer = mem_alloc(screen_buffer_size))) {
    fprintf(stderr, "Error: Not enough memory for screen buffer.\n");
    EXIT(1);
  }
#else
  w = w;
...
void c_setcolor_bg(unsigned char fg, unsigned char bg) {
  ...
  if (!(fg>>3) && !bg) {
    snprintf(txt, 16, "\033[%c;%dm", (fg >> 3) + '0', 30 + (fg & 7));
  } else {
    snprintf(txt, 16, "\033[%c;%d;%dm", (fg >> 3) +'0', 40 + (bg & 7), 30 + (fg & 7));
  }
  my_print(txt);
}
